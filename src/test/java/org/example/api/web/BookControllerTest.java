...
class BookControllerTest {
  ...
  void updateBookReturnsUpdatedBook() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/book/{id}", bookId).contentType("application/json").content(requestJson))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(bookId.toString()))
  ...
  void patchBookReturnsPatchedBook() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/v1/book/{id}", bookId).contentType("application/json").content(requestJson))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(bookId.toString()))
  ...
  void deleteBookReturnsOk() throws Exception {
    ...
    mockMvc.perform(delete("/api/v1/book/{id}", bookId)).andExpect(status().isNoContent());
  ...
  private String toRequestJson(
      String title, String subTitle, String description, int pages, String isbn) {
    return """
        {
          "title":"%s",
          "subTitle":"%s",
