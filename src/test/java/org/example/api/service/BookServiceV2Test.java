...
class BookServiceV2Test {
	...
	void createBookWithPublisherAndAuthors() {
		...
		when(authorRepository.findAllById(List.of(author1Id, author2Id))).thenReturn(List.of(author1, author2));
		when(bookRepository.save(any(Book.class)))
				.thenAnswer(invocation -> {
					Book savedBook = invocation.getArgument(0);
					savedBook.setId(bookId);
	...
	void patchBook() {
		...
		BookRequestDto request = new BookRequestDto(null, "new title", null, null, null, null, null, null);
