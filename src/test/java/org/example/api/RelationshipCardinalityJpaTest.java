...
class RelationshipCardinalityJpaTest {
  ...
  /**
   * Clears transactional contexts completely. This eliminates any false tracking positives and
   * guarantees Hibernate runs authentic 'SELECT' statements against the test database for
   * assertions.
   */
  ...
  class OneToManyPublisherBookTests {
    ...
    @Test
    @DisplayName(
        "Should associate multiple books with a single publisher (Cardinality Verification)")
    void shouldAssociateMultipleBooksWithSamePublisher() {
      ...
      assertEquals(
          2, foundPublisher.getBooks().size(), "Publisher should be mapped to exactly 2 books");
